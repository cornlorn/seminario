...
export const verificarToken = (request, response, next) => {
    ...
    if (!authHeader || !authHeader.startsWith("Bearer ")) {
        return response.status(401).json({ mensaje: "Token no proporcionado o formato inválido" });
    }
...
export const esAdminOEmpleado = (request, response, next) => {
    if (!["Administrador", "Empleado"].includes(request.usuario.permiso)) {
        return response
            .status(403)
            .json({
                mensaje: "Acceso denegado. Se requieren privilegios de administrador o empleado",
            });
    }
    next();
};
...
export const soloRegistroCliente = (request, response, next) => {
    if (request.body.permiso && request.body.permiso !== "Cliente") {
        return response
            .status(403)
            .json({ mensaje: "Solo se pueden registrar cuentas de cliente públicamente" });
    }
    next();
};
